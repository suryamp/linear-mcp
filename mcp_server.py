...
def list_workflow_states(team_id: str) -> str:
    ...
    """
    return json.dumps(linear.list_workflow_states(team_id))
...
def list_issues(
    ...
    """
    return json.dumps(linear.list_issues(team_id=team_id, assignee_id=assignee_id, state=state, limit=limit))
...
def get_issue(issue_id: str) -> str:
    ...
    """
    return json.dumps(linear.get_issue(issue_id), default=str)
...
def search_issues(query: str, limit: int = 25) -> str:
    ...
    Args:
        ...
        limit: Max results (default 25).
    """
    return json.dumps(linear.search_issues(query, limit))
...
def create_issue(
    ...
    """
    return json.dumps(linear.create_issue(
...
def update_issue(
    ...
    Args:
        ...
        priority: 0=None 1=Urgent 2=High 3=Medium 4=Low.
    """
    return json.dumps(linear.update_issue(
...
def delete_issue(issue_id: str) -> str:
    ...
    Args:
        issue_id: Issue UUID or identifier like 'ENG-42'.
    """
    return json.dumps({"success": linear.delete_issue(issue_id)})
...
def add_comment(issue_id: str, body: str) -> str:
    ...
    """
    return json.dumps(linear.add_comment(issue_id, body), default=str)
...
def list_projects(team_id: str = None) -> str:
    ...
    Args:
        team_id: Filter by team UUID.
    """
    return json.dumps(linear.list_projects(team_id))
